...
@interface KSCellAlignmentFlowLayout(){
    //在居中对齐的时候需要知道这行所有cell的宽度总和
    CGFloat _sumCellWidth ;
}
@end

@implementation KSCellAlignmentFlowLayout

-(instancetype)init{
    return [self initWithType:KSCellAlignmentCenter spacing:5.0];
}

-(void)setSpacing:(CGFloat)betweenOfCell{
    _spacing = betweenOfCell;
    self.minimumInteritemSpacing = betweenOfCell;
}

-(instancetype)initWthType:(KSCellAlignment)cellType{
    return [self initWithType:cellType spacing:5.0];
}
...
-(void)setCellFrameWith:(NSMutableArray*)layoutAttributes{
    ...
    switch (_cellType) {
        case KSCellAlignmentLeft:
            nowWidth = self.sectionInset.left;
            for (UICollectionViewLayoutAttributes * attributes in layoutAttributes) {
                CGRect nowFrame = attributes.frame;
                nowFrame.origin.x = nowWidth;
                attributes.frame = nowFrame;
                nowWidth += nowFrame.size.width + self.spacing;
            }
            _sumCellWidth = 0.0;
            [layoutAttributes removeAllObjects];
            break;
            
        case KSCellAlignmentCenter:
            nowWidth = (self.collectionView.frame.size.width - _sumCellWidth - ((layoutAttributes.count - 1) * _spacing)) / 2;
            for (UICollectionViewLayoutAttributes * attributes in layoutAttributes) {
                CGRect nowFrame = attributes.frame;
                nowFrame.origin.x = nowWidth;
                attributes.frame = nowFrame;
        ...
        case KSCellAlignmentRight:
            ...
            for (NSInteger index = layoutAttributes.count - 1 ; index >= 0 ; index-- ) {
                ...
                attributes.frame = nowFrame;
                nowWidth = nowWidth - nowFrame.size.width - _spacing;
            }
            _sumCellWidth = 0.0;
            [layoutAttributes removeAllObjects];
            break;
