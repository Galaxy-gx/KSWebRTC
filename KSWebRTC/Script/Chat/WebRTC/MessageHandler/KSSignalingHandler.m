...
@interface KSSignalingHandler()<KSWebSocketDelegate>
...
@end
@implementation KSSignalingHandler
...
- (void)configureRoom:(NSNumber *)handleId {
    KSMediaConnection *mc             = [self.delegate localPeerConnectionOfSignalingHandler:self handleId:handleId];
    mc.handleId                       = [handleId longLongValue];
    __weak KSSignalingHandler *weakSelf = self;
...
- (void)subscriberHandlerRemoteJsep:(NSNumber *)handleId dict:(NSDictionary *)jsep {
    KSMediaConnection *mc             = [self.delegate peerConnectionOfSignalingHandler:self handleId:handleId];
...
- (void)socketDidOpen:(KSWebSocket *)socket {
    NSLog(@"连接socket服务器成功");
    [self.delegate signalingHandler:self socketDidOpen:_socket];
    [self createSession];
}
...
- (void)socketDidFail:(KSWebSocket *)socket {
    NSLog(@"socket发生错误");
    [self.delegate signalingHandler:self socketDidFail:_socket];
}
