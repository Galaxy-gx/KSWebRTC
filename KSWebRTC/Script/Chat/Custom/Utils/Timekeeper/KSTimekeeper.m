...
-(void)countdownOfSecond:(int)second callback:(void(^)(BOOL isEnd))callback {
    NSDate *targetDate            = [NSDate dateWithTimeIntervalSinceNow:0];
    NSTimeInterval targetInterval = [targetDate timeIntervalSince1970] + second;
    __weak typeof(self) weakSelf  = self;
    if (_timer == nil) {
        __block int timeSign = second;
        if (timeSign != 0) {
            dispatch_queue_t queue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
            _timer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0,queue);
            dispatch_source_set_timer(_timer,dispatch_walltime(NULL, 0),1*NSEC_PER_SEC, 0);
...
-(void)timingOfCallback:(void(^)(KSTimekeeperInfo *timing))callback {
    ...
    [self timingOfTime:(int)startInterval callback:callback];
}
